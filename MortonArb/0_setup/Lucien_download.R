#Currently working with daily data because I don't have server access for some reason
#Eventually will pull hourly data so we can have min mean and max at the daily
...
all.day <- data.frame(model=as.factor(rep(mods.raw, each=length(yrs.all)*length(doy.all)*length(scen.all)*length(vars.all))),
                      scenario=as.factor(rep(scen.all, each=length(yrs.all)*length(doy.all)*length(vars.all))),
                      var=as.factor(rep(vars.all, each=length(yrs.all)*length(doy.all))),
                      year=rep(yrs.all, each=length(doy.all)),
                      yday=rep(doy.all, length.out=length(scen.all)*length(mods.raw)*length(vars.all)*length(doy.all)*length(yrs.all)))
...
for(MOD in mods.raw){
  ...
  for(SCEN in scenarios){
    ...
    mod.day <- array(dim=c(dim(mod.array)[c(1,2,3)], 3))
    
    dimnames(mod.day)[[1]] <- dimnames(mod.array)[[1]]
    dimnames(mod.day)[[2]] <- dimnames(mod.array)[[2]]
    dimnames(mod.day)[[3]] <- dimnames(mod.array)[[3]]
    dimnames(mod.day)[[4]] <- c("mean", "min", "max")
    
    mod.day[,,,1] <- apply(mod.array, c(1,2,3), mean)
    mod.day[,,,2] <- NA
    mod.day[,,,3] <- NA
    #mod.day[,,,2] <- apply(mod.array, c(1,2,3), min)
    #mod.day[,,,3] <- apply(mod.array, c(1,2,3), max)
    
    # Merge at least th mean data into the data frame
    for(VAR in vars.all){
      for(YEAR in yrs.all){
      ind.day <- which(all.day$model==MOD & all.day$scenario==SCEN & all.day$var==VAR & all.day$year == YEAR)
...
write.csv(all.day, "../Full_Weater_Daily.csv", row.names=F)
