...
#Emissions scenario table
checks <- c("2050", "2099")
met.var <- c("diff.tair", "diff.precip", "diff.VPD")
rcp.list <- list()
for(RCP in unique(runs.yr$rcp)){
  for(YR in checks){
    for(VAR in met.var){
...
#Emissions scenario table
checks <- c("2050", "2099")
met.var <- c("rel.tair", "rel.precip", "rel.VPD")
rcp.list <- list()
for(RCP in unique(runs.yr$rcp)){
  for(YR in checks){
    for(VAR in met.var){
...
write.csv(rcp.wide, "../data/RCP_relmet_values.csv")
#-----------------------------------------------------------#
...
crash.comp$sig <- ifelse(crash.comp$pvalue<=.05,"*"," ")
...
dat.harvest.end$time <- "End-of-century"
...
met.labs <- c("Temperature (C)", "Total precip (m)", "VPD (kPa)")
...
weath.cent <- ggplot(data=dat.wagg[dat.wagg$year == 2025 | dat.wagg$year == 2050 | dat.wagg$year == 2099,])+
  facet_grid(ind~., scales= "free_y", labeller = labeller(ind = met.labs), switch = "y") +
