...
#' Write ED meteorlogy header with static CO2
#' Write ED met driver header from R met driver list object
#'
#' @param ed_metheader ED meteorlogy header object (see [read_ed_metheader])
#' @param filename Full file name (including path) of ED met header
...
#' `'header'`.
#' @export
write_ed_metheader_statCO2 <- function(ed_metheader, filename,
                                       header_line = shQuote("header")) {
  nformats <- length(ed_metheader)
  blocks <- vector("list", nformats)
  for (i in seq_len(nformats)) {
    ...
    # Find which one is CO2 and manually overwrite some info:
    ind.co2 <- which(metformat$variables$variable=="co2")
    metformat$variables$update_frequency[ind.co2] <- 380
    metformat$variables$flag[ind.co2] <- 4
